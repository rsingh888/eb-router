...
// Check if settings has ebRouter customModels
const has9RouterConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:ebRouter"));
};
...
// POST - Update ebRouter customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing ebRouter configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:ebRouter"));
...
// DELETE - Remove ebRouter customModels only (keep other settings)
