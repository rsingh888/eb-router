...
const has9RouterConfig = (auth) => {
  ...
  const entry = auth["openai-compatible"] || auth["ebrouter"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("9router") || baseUrl.includes("ebrouter");
};
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "ebRouter", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "ebRouter settings removed from Kilo Code" });
  } catch (error) {
