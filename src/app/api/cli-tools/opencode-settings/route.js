...
const has9RouterConfig = (config) => {
  ...
  return !!(config.provider["ebrouter"] || config.provider["9router"]);
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["ebrouter"];
    ...
    return NextResponse.json({
      ...
      configPath: getConfigPath(),
        opencode: {
          ...
          activeModel: /^(?:9router|ebrouter)\//.test(config?.model || "") ? config.model.replace(/^(?:9router|ebrouter)\//, "") : null,
...
// POST - Apply ebRouter as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_ebrouter";
    ...
    const existingProvider = config.provider["ebrouter"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["ebrouter"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `ebrouter/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `ebrouter/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("ebrouter/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove ebRouter provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["ebrouter"]?.models) {
      delete config.provider["ebrouter"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["ebrouter"].models).length === 0) {
        delete config.provider["ebrouter"];
        if (config.model?.startsWith("ebrouter/")) delete config.model;
      } else if (config.model === `ebrouter/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["ebrouter"].models);
        config.model = `ebrouter/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire 9router provider
      if (config.provider) delete config.provider["ebrouter"];
      if (config.model?.startsWith("ebrouter/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("ebrouter/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "ebRouter settings removed from OpenCode",
    });
  } catch (error) {
