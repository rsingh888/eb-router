...
const has9RouterConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "ebRouter");
};

const get9RouterEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "ebRouter") || null;
};
...
// POST - Apply ebRouter config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing ebRouter entry or append
    const idx = config.findIndex((e) => e.name === "ebRouter");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove ebRouter entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "ebRouter");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "ebRouter removed from Copilot config",
    });
  } catch (error) {
