...
export const GET = withAuthUser(async (_request, _ctx, user) => {
  try {
    const models = await getCustomModels(user.id);
...
export const POST = withAuthUser(async (request, _ctx, user) => {
  try {
    ...
    const added = await addCustomModel(user.id, { providerAlias, id, type: type || "llm", name });
...
export const DELETE = withAuthUser(async (request, _ctx, user) => {
  try {
    ...
    await deleteCustomModel(user.id, { providerAlias, id, type });
