...
<div align="center">
  <img src="../images/ebrouter.png?1" alt="Bảng điều khiển ebRouter" width="800"/>
  
  # ebRouter - Free AI Router
  ...
  **Nhà cung cấp AI Miễn cho OpenClaw.**
  ...
  [🚀 Bắt đầu nhanh](#-quick-start) • [💡 Tính năng](#-key-features) • [📖 Cài đặt](#-setup-guide) • [🌐 Website](https://ebrouter.example.com)
</div>

---

## 🤔 Tại sao chọn ebRouter?
...
**ebRouter giải quyết vấn đề này:**
...
Gói kho lưu trữ này là riêng tư (`ebrouter-app`), vì vậy việc thực thi nguồn/Docker là đường dẫn phát triển cục bộ dự kiến.
...
### 📺 Hướng dẫn thiết lập hoàn chỉnh - ebRouter + Claude Code MIỄN PHÍ
  
[![Thiết lập ebRouter + Claude Code](https://img.youtube.com/vi/raEyZPg5xE0/maxresdefault.jpg)](https://www.youtube.com/watch?v=raEyZPg5xE0)

**🎬 Xem hướng dẫn từng đầy đủ:**
- ✅ Cài đặt & thiết lập ebRouter
...
ebRouter hoạt động liền mạch với tất cả các công cụ code AI chính:
...
- Công cụ CLI của bạn gửi định dạng OpenAI → ebRouter dịch chuyển → Nhà cung cấp nhận định dạng gốc
...
> Bản thân ebRouter **không bao giờ thu phí** bạn bất cứ thứ gì. Bạn chỉ trả tiền trực tiếp cho các nhà cung cấp (nếu sử dụng dịch vụ trả phí).
> 
...
### 📊 Hiểu về Chi phí & Thanh toán của ebRouter

**Thực tế Thanh toán ebRouter:**

✅ **Phần mềm ebRouter = MIỄN PHÍ mãi mãi** (mã nguồn mở, không bao giờ thu phí)  
...
❌ **ebRouter không bao giờ gửi hóa đơn** hoặc tính phí thẻ của bạn
...
- **Nhà cung cấp giá rẻ** (GLM, MiniMax): Trả tiền trực tiếp cho họ, ebRouter chỉ định tuyến
...
<details>
<summary><b>💳 Tôi có bị ebRouter tính phí không?</b></summary>

**Không.** ebRouter là phần mềm miễn phí, mã nguồn mở chạy trên máy tính của chính bạn. Nó không bao giờ tính phí bạn bất cứ thứ gì.

**Bạn chỉ trả tiền:**
- ✅ **Nhà cung cấp gói đăng ký** (Claude Code $20/tháng, Codex $20-200/tháng) → Trả tiền trực tiếp cho họ trên website của họ
- ✅ **Nhà cung cấp giá rẻ** (GLM, MiniMax) → Trả tiền trực tiếp cho họ, ebRouter chỉ định tuyến yêu cầu của bạn
- ❌ **Bản thân ebRouter** → **Không bao giờ tính phí bất cứ thứ gì, ever**

ebRouter là một proxy/router cục bộ. Nó không cóẻ tín dụng của bạn, không thể gửi hóa đơn và không có hệ thống thanh toán. Đó là phần mềm hoàn toàn miễn phí.
...
ebRouter chỉ định tuyến yêu cầu của bạn đến họ - không có "cạm bẫy" hay thanh toán trong tương lai. Đó là các dịch vụ thực sự miễn phí, và ebRouter giúp chúng dễ sử dụng với hỗ trợ dự phòng.

**Lưu ý:** số nhà cung cấp gói đăng ký (Antigravity, GitHub Copilot) có thể có các khoảng thời gian dùng thử miễn phí có thể trở thành trả phí sau này, nhưng điều này sẽ được các nhà cung cấp đó thông báo rõ ràng, không phải ebRouter.
...
3. **Sử dụng nhà cung cấp gói đăng ký cuối cùng:**
   ...
   - ebRouter giúp tối đa hóa giá trị của chúng thông qua theo dõi hạn mức
...
Cơ chế dự phòng thông minh của ebRouter ngăn chặn các khoản phí bất ngờ:
...
**Bạn nắm quyền kiểm soát:** Đặt giới hạn chi tiêu cho mỗi nhà cung cấp trong bảng điều khiển, và ebRouter sẽ tuân thủ chúng.
...
**Mẹo Chuyên nghiệp:** Sử dụng Opus cho các tác vụ phức tạp, Sonnet cho tốc độ. ebRouter theo dõi hạn mức cho mỗi mô hình!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [từ bảng điều khiển ebrouter]
...
> **Lưu ý:** OpenClaw chỉ hoạt động với ebRouter cục bộ. Sử dụng `127.0.0.1` thay vì `localhost` để tránh các vấn đề phân giải6.
...
| `CLOUD_URL` | `https://ebrouter.example.com` | URL cơ sở endpoint đồng bộ đám mây phía máy chủ |
| `NEXT_PUBLIC_BASE_URL` | `http://localhost:3000` | URL cơ sở tương thích ngược/công khai (ưu tiên `BASE_URL` cho runtime máy chủ) |
| `NEXT_PUBLIC_CLOUD_URL` | `https://ebrouter.example.com` | URL đám mây tương thích ngược/công khai (ưu tiên `CLOUD_URL` cho runtime máy chủ) |
...
- Xác minh `CLOUD_URL` trỏ đến endpoint đám mây dự kiến của bạn (ví dụ: `https://ebrouter.example.com`)
...
- Triệu chứng thường xuất hiện trên endpoint đám mây công khai (`https://ebrouter.example.com/v1`) cho các lệnh gọi không phát trực tiếp (non-streaming).
...
- Runtime ebRouter cục bộ bao gồm dự phòng SSE→JSON cho các lệnh gọi không phát trực tiếp khi upstream trả về `text/event-stream`.
...
- `tester/security/test-cloud-openai-compatible.sh`
  - Gửi một yêu cầu tương thích OpenAI trực tiếp đến endpoint đám mây (`https://ebrouter.example.com/v1/chat/completions`) với mô hình/key được cung cấp.
...
- Endpoint đám mây công khai (`https://ebrouter.example.com/v1/chat/completions`):
...
Cảm ơn tất cả những người đã đóng góp giúp ebRouter tốt hơn!

[![Contributors](https://contrib.rocks/image?repo=YOUR_ORG/ebRouter&max=100&columns=20&anon=1)](https://github.com/YOUR_ORG/ebRouter/graphs/contributors)

---

## 📊 Star Chart

[![ Chart](https://starchart.cc/YOUR_ORG/ebRouter.svg?variant=adaptive)](https://starchart.cc/YOUR_ORG/ebRouter)
...
Xem [Pull Requests](https://github.com/YOUR_ORG/ebRouter/pulls) để biết hướng dẫn chi tiết.

---

## 🔀 Forks

**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — Một fork TypeScript đầy đủ tính năng của ebRouter. Thêm 36+ nhà cung cấp, tự động dự phòng 4 tầng, API đa phương thức (hình ảnh, embedding, âm thanh, TTS), circuit breaker, bộ nhớ đệm ngữ nghĩa, đánh giá LLM và bảng điều khiển được tinh chỉnh. 368+ bài kiểm tra đơn vị. Có sẵn qua npm và.
