---
name: ebrouter-tts
description: Text-to-speech via ebRouter /v1/audio/speech using OpenAI / ElevenLabs / Deepgram / Edge TTS / Google TTS / Hyperbolic / Inworld voices. Use when the user wants to convert text to speech, generate audio, voiceover, narrate, or read text aloud.
---

# ebRouter — Text-to-Speech

Requires `EBROUTER_URL` (and `EBROUTER_KEY` if auth enabled). See https://raw.githubusercontent.com/YOUR_ORG/ebRouter/refs/heads/master/skills/ebrouter/SKILL.md for setup.

## Discover

```bash
# 1) List models
curl $EBROUTER_URL/v1/models/tts | jq '.data[].id'
# 2) Per-model metadata (params, voicesUrl if voice-by-id)
curl "$EBROUTER_URL/v1/models/info?id=el/eleven_multilingual_v2"
# 3) List voices (elevenlabs, edge-tts, deepgram, inworld, local-device). Optional ?lang=vi
curl "$EBROUTER_URL/v1/audio/voices?provider=edge-tts&lang=vi" | jq '.data[].model'
```
...
`POST $EBROUTER_URL/v1/audio/speech`
...
```bash
curl -X POST "$EBROUTER_URL/v1/audio/speech" \
  -H "Authorization: Bearer $EBROUTER_KEY" \
...
const r = await fetch(`${process.env.EBROUTER_URL}/v1/audio/speech`, {
  method: "POST",
  headers: { "Authorization": `Bearer ${process.env.EBROUTER_KEY}`, "Content-Type": "application/json" },
