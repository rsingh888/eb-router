...
function buildMenuItems(port, autostartEnabled) {
  return [
    { title: `ebRouter (Port ${port})`, tooltip: "Server is running", enabled: false },
  ...
  try {
    ...
    if (isV2) {
      ...
      trayInstance.ready().catch((err) => {
        process.stderr.write(`[ebrouter] tray failed to start: ${err && err.message ? err.message : err}\n`);
      });
    } else {
    ...
    process.stderr.write(`[ebrouter] tray init error: ${err.message}\n`);
    return null;
  }
}
